...
# Precision-Recall AUC for multiclass only
if len(np.unique(y)) > 2:
    logger.info("Calculating Precision-Recall AUC for multiclass...")
    y_test_binarized = label_binarize(y_test, classes=np.unique(y))
    y_pred_binarized = label_binarize(y_pred, classes=np.unique(y))
    average_precision = average_precision_score(
        y_test_binarized, y_pred_binarized, average="macro"
    )
    logger.info(f"Average Precision Score (Macro): {average_precision:.2f}")
else:
    logger.info("Skipping Precision-Recall AUC calculation for binary classification.")
